...
function brandFromName(name = '', category = '') {
  ...
  if (/autocad|auto\s*cad/.test(text)) {
    return { brand: 'Autodesk', product: 'AutoCAD', from: '#0E5A8A', to: '#00E0C6', icon: 'cad' }
  }
  if (/revit|bim/.test(text)) {
    return { brand: 'Autodesk', product: 'Revit', from: '#1F4E79', to: '#3AA6D9', icon: 'building' }
  }
  if (/maya|mudbox|arnold|flame/.test(text)) {
    return { brand: 'Autodesk', product: 'Media & Entertainment', from: '#111827', to: '#7C3AED', icon: 'box' }
  }
  if (/fusion|inventor|navisworks|alias|forma|vault|recap|autodesk/.test(text)) {
    return { brand: 'Autodesk', product: 'Design Suite', from: '#0696D7', to: '#0B3C5D', icon: 'box' }
  }
  if (/corel/.test(text)) {
    return { brand: 'Corel', product: 'CorelDRAW', from: '#00A651', to: '#004B2E', icon: 'box' }
  }
  if (/adobe|photoshop|illustrator|acrobat/.test(text)) {
    return { brand: 'Adobe', product: 'Creative', from: '#FF0000', to: '#4B0000', icon: 'box' }
  }
  if (/visual\s*studio/.test(text)) {
    return { brand: 'Microsoft', product: 'Visual Studio', from: '#5C2D91', to: '#1B0A33', icon: 'code' }
  }
  if (/sql\s*server/.test(text)) {
    return { brand: 'Microsoft', product: 'SQL Server', from: '#CC2927', to: '#4A0D0C', icon: 'db' }
  }
  if (/windows\s*server|datacenter/.test(text)) {
    return { brand: 'Microsoft', product: 'Windows Server', from: '#0078D4', to: '#001B3D', icon: 'server' }
  }
  if (/windows/.test(text)) {
    return { brand: 'Microsoft', product: 'Windows', from: '#0078D4', to: '#00BCF2', icon: 'windows' }
  }
  if (/\bexcel\b/.test(text)) {
    return { brand: 'Microsoft', product: 'Excel', from: '#217346', to: '#0B3D24', icon: 'sheet' }
  }
  if (/\bword\b/.test(text)) {
    return { brand: 'Microsoft', product: 'Word', from: '#2B579A', to: '#0F274D', icon: 'doc' }
  }
  if (/powerpoint|power\s*point/.test(text)) {
    return { brand: 'Microsoft', product: 'PowerPoint', from: '#C43E1C', to: '#5A1408', icon: 'slides' }
  }
  if (/outlook/.test(text)) {
    return { brand: 'Microsoft', product: 'Outlook', from: '#0072C6', to: '#00335C', icon: 'mail' }
  }
  if (/visio/.test(text)) {
    return { brand: 'Microsoft', product: 'Visio', from: '#3955A3', to: '#1A274D', icon: 'flow' }
  }
  if (/\bproject\b/.test(text)) {
    return { brand: 'Microsoft', product: 'Project', from: '#31752F', to: '#143312', icon: 'chart' }
  }
  if (/\boffice\b|microsoft\s*365|\b365\b|mak/.test(text)) {
    return { brand: 'Microsoft', product: 'Office', from: '#D83B01', to: '#7A1F00', icon: 'office' }
  }
  if (/antivirus|kaspersky|mcafee|norton|security|defender/.test(text)) {
    return { brand: 'Security', product: 'Antivirus', from: '#0F766E', to: '#042F2E', icon: 'shield' }
  }
  if (/server/.test(text) || /server/i.test(category)) {
    return { brand: 'Enterprise', product: 'Server', from: '#334155', to: '#0F172A', icon: 'server' }
  }
  return {
    brand: 'eSoftware',
...
export function buildProductCoverSvg({ name = '', category = '', slug = '', productType = '' } = {}) {
  ...
  const brand = isBundle
    ? { brand: 'eSoftware', product: 'Bundle Deal', from: '#7c3aed', to: '#312e81', icon: 'bundle' }
...
export function productCoverDataUri(product) {
  const svg = buildProductCoverSvg({
    ...
    category: typeof product === 'string' ? '' : product?.category,
    slug: typeof product === 'string' ? '' : product?.slug || product?.id,
    productType: typeof product === 'string' ? '' : product?.productType,
  })
