...
/**
 * Stable cover image derived from product name (and category as fallback).
 */
export function getProductImageByName(productOrName, category = '', slug = '') {
  ...
  const key =
    typeof productOrName === 'string'
      ? slug || name
      : productOrName?.slug || productOrName?.id || name
...
/** Prefer a working custom URL; otherwise name-based cover. */
